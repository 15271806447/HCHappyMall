Page({
  ...
  validatedCart: function() {
    var that = this;
    for(let i=0;i<that.data.cart.length;i++){
      wx.request({
        url: 'https://ccdcapi.alipay.com/validateAndCacheCardInfo.json?cardNo=' + that.data.cart[i].cartNo+'&cardBinCheck=true',
        header: {
        ...
        success: function (res) {
          console.log(res);
          var key = that.findBank(res.data.bank);
          console.log(key);
        }
      })
    }
